<?php
namespace Z\Core\DB\Provider\MySQLi;

class Result extends \Z\Core\DB\Result {
	protected $result;
	...
	public function current() {
		if ($this->valid()) {
			if (!$this->result->data_seek($this->cursor))
				throw new \Z\Core\DB\Exception("Can't seek result to ".$this->cursor." item, but total rows is ".$this->total_rows);
