...
class BulkInsert extends \Z\Core\DB\Builder {
	...
	public function values($arr) {
		if (count($arr) != count($this->fields))
			throw new \Z\Core\DB\Exception("Values count (".count($arr).") does not match fields count (".count($this->fields).").");
		$this->values[] = $arr;
		return $this;
	}
	...
	public function valuesAssoc($arr) {
		...
		foreach ($this->fields as $k) {
			if (!array_key_exists($k, $arr))
				throw new \Z\Core\DB\Exception("Key `$k` not found in values.");
			$value[] = $arr[$k];
		}
		$this->values[] = $value;
		return $this;
