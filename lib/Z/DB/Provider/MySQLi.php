<?php
namespace Z\DB\Provider;

class MySQLi extends \Z\DB\Provider {
	protected $config;
	protected $handle;
	...
	private function connection() {
		if (!$this->handle) {
			...
			if ($this->handle->connect_error)
				throw new \Z\DB\Exception("MySQLi connect error (".$this->handle->connect_errno."): ".$this->handle->connect_error);
			
			if (!$this->handle->select_db($this->config['database']))
				throw new \Z\DB\Exception("Select db ".$this->config['database'].": #".$this->handle->errno." ".$this->handle->error);
			
			if (!$this->handle->set_charset($this->config['charset']))
				throw new \Z\DB\Exception("Set charset: ".$this->config['charset']." #".$this->handle->errno." ".$this->handle->error);
		}
	...
	public function query($query) {
		...
		if ($result === false)
			throw new \Z\DB\Exception("#".$this->handle->errno." ".$this->handle->error." [ $query ]");
