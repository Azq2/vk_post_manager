...
class Downloader extends \Z\Core\Task {
	...
	public function run($args) {
		...
		$this->api = new \Z\Core\Net\VkApi(\Z\Smm\Oauth::getAccessToken('VK'));
		
		\Z\Core\Util\Inotify::watch(self::QUEUE_DIR, [$this, 'processQueue']);
	...
	public function queueDownloadFiles($id) {
		...
		if ($queue->cover) {
			$cover = \Z\Smm\Utils\GD::imageCreateFromFile($queue->cover);
			if (!$cover) {
			...
			$this->download_ids[$queue->id][] = $this->download($file['url'], $file['out'], function ($path) use ($file, $queue, $cover) {
				...
				if ($file['type'] == 'photo') {
					...
					$url = DB::select('url')
						...
						->where('id', '=', \Z\Smm\Grabber::SOURCE_VK.'_-'.$queue->gid)
						->execute()
						->get('url', false);
				...
				while ($tries) {
					$result = \Z\Smm\VK\Posts::uploadPics($this->api, $queue->gid, $upload);
					if ($result->success) {
	...
	public function queueDone($id) {
		...
		$queue->done = true;
		$queue->attaches_data = \Z\Smm\VK\Posts::normalizeAttaches((object) [
	...
	public function imageWatermark($tmp_file, $cover, $offset) {
		$image = \Z\Smm\Utils\GD::imageCreateFromFile($tmp_file);
		if (!$image)
