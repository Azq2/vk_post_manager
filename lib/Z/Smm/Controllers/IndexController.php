...
class IndexController extends \Z\Smm\GroupController {
	...
	public function suggestedAction() {
		...
		$res = \Z\Smm\VK\Posts::getAll($api, $this->group['id']);
		...
		if ($res->success) {
			foreach ($res->{$list} as $item) {
				...
				$attaches_info = \Z\Smm\VK\Posts::normalizeAttaches($item);
	...
	public function multipicpostAction() {
		...
		if ($_FILES && isset($_FILES['file'])) {
			...
			if (!$this->user->can('user')) {
				...
				if ($result->success) {
					$queue_result = \Z\Smm\VK\Posts::queueWallPost($this->group['id'], array_keys($result->attachments), $_POST['message'] ?? "");
